...
void Model::draw()
{
	GLuint model_location = glGetUniformLocation(Shader::get_current_shader(), "model");
	glUniformMatrix4fv(model_location, 1, GL_FALSE, glm::value_ptr(transform.get_transformation()));

	if (skeleton.size() > 0)
	{
		GLuint bone_loc = glGetUniformLocation(Shader::get_current_shader(), "bone_mats");
		glUniformMatrix4fv(bone_loc, skeleton.size(), GL_FALSE, glm::value_ptr(skeleton.bone_transforms[0]));

		GLint flag_loc = glGetUniformLocation(Shader::get_current_shader(), "has_bones");
